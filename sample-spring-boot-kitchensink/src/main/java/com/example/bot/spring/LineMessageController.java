...
/**
 * @author Group 16 This class is Line Message Controller
 */
...
public class LineMessageController {
	...
	public void handlePostbackEvent(PostbackEvent event) throws Exception {
		String replyToken = event.getReplyToken();
		/*
		 * String reply=""; String text=event.getPostbackContent().getData(); if
		 * (text.contains(Constant.TEXT_NEW_BOOKING)){ listTourForBooking(replyToken,
		 * reply); }
		 */
	 ...
	 * @param reply
	 */
	public void FAQ_NO_CONFIRMATION_WITH_USER_INFORMATION_handler(String replyToken, String text, String userID,
	...
	public void FILL_NAME_handler(String replyToken, String text, String userID, String reply) throws Exception {
	...
	public void FILL_PHONE_NUM_handler(String replyToken, String text, String userID, String reply) throws Exception {
	...
	public void BOOKING_OFFERING_ID_handler(String replyToken, String text, String userID, String reply)
		...
		if (!checkQuit(text, userID, reply, replyToken, Constant.DELETING_BOOKING_BUFFER)) {
			if (isNumeric(text)
					&& database.tourOfferingFound(database.getBufferTourID(userID), Integer.parseInt(text))) {
	 ...
	 * @param reply
	 */
	public void BOOKING_CHILDREN_handler(String replyToken, String text, String userID, String reply) throws Exception {
		if (!checkQuit(text, userID, reply, replyToken, Constant.DELETING_BOOKING_ENTRY)) {
			text = text.replaceAll(" ", "");
			if (isNumeric(text) && Integer.parseInt(text) >= 0) {
		...
		if (!checkQuit(text, userID, reply, replyToken, Constant.DELETING_BOOKING_ENTRY)) {
			...
			if (isNumeric(text) && Integer.parseInt(text) >= 0) {
				...
				if (quota >= 0) {
					...
					log.info("Returns instruction message {}: {}", replyToken, reply);
					this.reply(replyToken, Arrays.asList(new TextMessage(Constant.INSTRTUCTION_ENTER_SPECIAL_REQUEST),
							ButtonMessageBlock));
	...
	public void BOOKING_CONFIRMATION_handler(String replyToken, String text, String userID, String reply)
		...
		if (!checkQuit(text, userID, reply, replyToken, Constant.DELETING_BOOKING_ENTRY)) {
			...
			this.reply(replyToken, Arrays.asList(new TextMessage(reply), confirmMessageBlock));
		}
	}
	...
	private List<Message> splitMessages(String longstring, String splitter) {
		if (longstring != null) {
			...
			for (int i = 0; i < shortStrings.length; i++) {
				...
				} else if (i + 1 == shortStrings.length) { // dealing with boundary case, e.g 27/5=5 5+1=6, the last one
															// does not give 0
					Message message = new TextMessage(groupString);
					messages.add(message);
					groupString = "";// clear the groupString
	...
	public void listTourForBooking(String replyToken, String reply) throws Exception {
		...
		for (Tour tour : listOfTours) {
			...
			if (trancatedDescription.length() > 60)
				trancatedDescription = trancatedDescription.substring(0, 60 - 2) + "..";
			CarouselColumn item = new CarouselColumn(imageUrl, tour.getTourName(), trancatedDescription,
	 ...
	 * @param path
	 */
	static public String createUri(String path) {
	...
	/**
	 * Constructor. The annontation @Value is from the package lombok.Value
	 * Basically what it does is to generate constructor and getter for the class
	 * below See https://projectlombok.org/features/Value
	 */
	...
	/**
	 * @author Group 16 This class is Profile Getter
	 */
